...
if __name__ == "__main__":
    ...
    page_rows_added = add_visual_titles_and_action_names_to_pages(
        page_rows, visual_title_map, page_id_to_name, bookmark_id_to_name
    )
    ...
    df_pages = pd.DataFrame(
        ...
        columns=[
                       "Page ID", "Page Name",
            "Visual ID", "Visual Title", "Visual Type",
            "Action Type", "Action Name",
            "Visual Columns", "Visual Filters"
        ]
    )

    df_visuals = visuals_df.copy()
    if df_visuals is None or df_visuals.empty:
        df_visuals = pd.DataFrame(columns=["Page ID", "Visual ID", "Visual Type", "Visual Title"])
